...
def phase_diagram(analysis_output, mp, mp_u, mp_concs, mp_concs_u):
    output_parent = analysis_output
    _, _, temps = parse(analysis_output)
    ...
    dendil.to_csv(f"{analysis_output}/dendil_data.csv")
    
    melt_point = pd.DataFrame({"conc": mp_concs, "conc_u": mp_concs_u,"mp": mp, "mp_u": mp_u})
    melt_point.to_csv(f"{analysis_output}/mp_data.csv")
    ...
    with plt.style.context(["science","nature"]):
        ...
        plt.savefig(f"{analysis_output}/phase_diagram.png")
