...
import pandas as pd
from lib.segmentation.dil_radius import dil_radii
from lib.segmentation.den_radius import den_radii
import matplotlib.pyplot as plt
console = Console()

class DropletImage:
    def __init__(self, img_path, feature_table_path, psf):
        ...
        den_radius_li = den_radii(self, drop_li, psf)
    ...
    def write_csv(self, path, name, min_drop_radius, max_drop_radius, cf1, cf2):
        ...
        for droplet in self.droplets:
            if droplet.dil_radius > min_drop_radius and droplet.dil_radius < max_drop_radius:
                x.append(droplet.x)
                y.append(droplet.y)
        ...
        self.log = {"corrected_rden": cf1 * r_den + cf2,"rden": r_den, "rdil": r_dil, "xcen": x, "ycen": y}
    ...
    def segmenation_image(self, path, name, min_drop_radius, max_drop_radius):
        ...
        for droplet in self.droplets:
            if droplet.dil_radius > min_drop_radius and droplet.dil_radius < max_drop_radius and droplet.den_radius < droplet.dil_radius:
                x.append(droplet.x)
                y.append(droplet.y)
                r_den.append(droplet.den_radius)
                r_dil.append(droplet.dil_radius)
